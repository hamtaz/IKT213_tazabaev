...
def padding(image, border_width):
    ...
    cv2.imwrite("output/padded.png", reflect)
...
def copy(image, emptyPictureArray):
    ...
    cv2.imwrite("output/copy.png", emptyPictureArray)
...
def hsv(image):
    ...
    cv2.imwrite("output/hsv.png", hsvImage)
...
def hue_shifted(image, emptyPictureArray, hue):
    ...
    cv2.imwrite("output/hue_shifted.png", emptyPictureArray)

def smoothing(image):
    ksize=(15,15)
    smoothed_image = cv2.GaussianBlur(image, ksize, 0)
    cv2.imwrite("output/smoothed.png", smoothed_image)

def rotation(image, rotation_angle):
    if rotation_angle == 90:
        rotated = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
        cv2.imwrite("output/90_rotated.png", rotated)
    elif rotation_angle == 180:
        rotated = cv2.rotate(image, cv2.ROTATE_180)
        cv2.imwrite("output/180_rotated.png", rotated)
